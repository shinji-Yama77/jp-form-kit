import type { OverlayFormSchema } from "../../types.js";

export const juminhyoSchema: OverlayFormSchema = {
  id: "juminhyo",
  titleJa: "住民票等請求書",
  titleEn: "Resident Record Request",
  pdfFilename: "juminhyo.pdf",
  sourceUrl:
    "https://www.city.minato.tokyo.jp/documents/9717/20260206101744.pdf",
  category: "ward",
  jurisdiction: "minato-ku",
  lastVerifiedAt: "2026-04-09",
  variants: [
    {
      lang: "en",
      pdfFilename: "juminhyo-en.pdf",
      sourceUrl:
        "https://www.city.minato.tokyo.jp/documents/9717/20260206101812.pdf",
      lastVerifiedAt: "2026-04-10",
      fields: [
        {
          key: "full_name",
          x: 83,
          y: 708,
          width: 291,
          height: 16,
          labelEn: "Full Name",
          labelJa: "氏名",
        },
        {
          key: "home_address",
          x: 91,
          y: 672,
          width: 480,
          height: 16,
          labelEn: "Home Address",
          labelJa: "住所",
        },
        {
          key: "contact_phone",
          x: 419,
          y: 639,
          width: 154,
          height: 16,
          labelEn: "Daytime Contact Phone Number",
          labelJa: "昼間連絡のつく電話番号",
        },
        {
          key: "counter_visitor_name",
          x: 63,
          y: 431,
          width: 310,
          height: 16,
          labelEn: "Counter Visitor Name",
          labelJa: "窓口に来られた方の氏名",
        },
        {
          key: "counter_visitor_address",
          x: 62,
          y: 391,
          width: 313,
          height: 16,
          labelEn: "Counter Visitor Address",
          labelJa: "窓口に来られた方の住所",
        },
        {
          key: "counter_visitor_phone",
          x: 420,
          y: 391,
          width: 153,
          height: 16,
          labelEn: "Counter Visitor Phone",
          labelJa: "窓口に来られた方の電話番号",
        },
        { key: "application_year", x: 324, y: 751, width: 84, height: 16 },
        { key: "application_day", x: 483, y: 751, width: 89, height: 16 },
        {
          key: "head_of_household_name",
          x: 418,
          y: 708,
          width: 156,
          height: 16,
          labelEn: "Head of Household Name",
          labelJa: "世帯主の氏名",
        },
        { key: "counter_visitor_dob", x: 420, y: 444, width: 153, height: 16 },
        {
          key: "requester_name",
          x: 85,
          y: 328,
          width: 289,
          height: 16,
          labelEn: "Requester Name",
          labelJa: "請求者の氏名",
        },
        {
          key: "requester_address",
          x: 83,
          y: 291,
          width: 293,
          height: 16,
          labelEn: "Requester Address",
          labelJa: "請求者の住所",
        },
        {
          key: "requester_phone",
          x: 420,
          y: 300,
          width: 153,
          height: 16,
          labelEn: "Requester Phone",
          labelJa: "請求者の電話番号",
        },
      ],
    },
  ],
  fields: [
    {
      key: "furigana",
      x: 84,
      y: 718,
      width: 282,
      height: 17,
      labelEn: "Name (Katakana)",
      labelJa: "フリガナ",
    },
    {
      key: "full_name",
      x: 85,
      y: 694,
      width: 289,
      height: 17,
      labelEn: "Full Name",
      labelJa: "氏名",
    },
    {
      key: "home_address",
      x: 128,
      y: 659,
      width: 448,
      height: 17,
      labelEn: "Home Address",
      labelJa: "住所",
    },
    {
      key: "contact_phone",
      x: 419,
      y: 626,
      width: 153,
      height: 17,
      labelEn: "Daytime Contact Phone Number",
      labelJa: "昼間連絡のつく電話番号",
    },
    {
      key: "head_of_household_name",
      x: 418,
      y: 694,
      width: 155,
      height: 17,
      labelEn: "Head of Household Name",
      labelJa: "世帯主の氏名",
    },
    {
      key: "counter_visitor_name",
      x: 84,
      y: 417,
      width: 291,
      height: 17,
      labelEn: "Counter Visitor Name",
      labelJa: "窓口に来られた方の氏名",
    },
    {
      key: "counter_visitor_address",
      x: 84,
      y: 372,
      width: 291,
      height: 17,
      labelEn: "Counter Visitor Address",
      labelJa: "窓口に来られた方の住所",
    },
    {
      key: "counter_visitor_phone",
      x: 418,
      y: 372,
      width: 155,
      height: 17,
      labelEn: "Counter Visitor Phone",
      labelJa: "窓口に来られた方の電話番号",
    },
    {
      key: "requester_name",
      x: 85,
      y: 315,
      width: 290,
      height: 17,
      labelEn: "Requester Name",
      labelJa: "請求者の氏名",
    },
    {
      key: "requester_address",
      x: 83,
      y: 280,
      width: 293,
      height: 17,
      labelEn: "Requester Address",
      labelJa: "請求者の住所",
    },
    {
      key: "requester_phone",
      x: 418,
      y: 288,
      width: 155,
      height: 17,
      labelEn: "Requester Phone",
      labelJa: "請求者の電話番号",
    },
  ],
};
